...
5. Run `python3 tools_check_docs.py` — it verifies every count `llms.txt`
   asserts against the live database and that `llms.html` mirrors the current
   text, and exits non-zero on any mismatch. When a documented number
   legitimately changes, update `llms.txt`, the checker's `CLAIMS` table, and
   the mirror (`python3 tools_make_llms_html.py`) in the same commit. This
   step exists because the file's DOI-defect paragraph survived three
