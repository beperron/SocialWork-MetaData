# SSWR — Semantic & Hybrid Search Recipes
...
# fields: id, title, abstract, year, methodology, similarity, authors (full author list as JSON)
```
...
```sql
select methodology, count(*) from sswr.papers
where id in ('2019-O-0142', '2024-P-0198') group by 1
```
...
```sql
select h.title, h.rrf_score, h.year
from sswr.search_papers_hybrid('kinship care', '[...768 floats...]'::extensions.vector, 20) h
where h.methodology = 'quantitative'
```
