...
Interpretation: `similarity` runs 0–1 (cosine), but read it relatively — the scale shifts with query length and specificity. A two-word query may top out near 0.56 while a long question puts every row above 0.55. Sort by score, find the elbow where values flatten, and read abstracts there. All search functions return the full author list as JSON. No Ollama available? `rpc/search_papers_keyword` needs no embedding and no setup.
...
| Symptom | Fix |
|---|---|
| Any error naming `public.` (`could not find the table 'public.papers'`, `could not find the function public.run_sql`) | The profile header is missing: `Accept-Profile: sswr` on GET, `Content-Profile: sswr` on POST |
