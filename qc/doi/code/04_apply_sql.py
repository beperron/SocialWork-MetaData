...
def main():
    ...
    lines = [
        ...
        "--",
        "-- The before/after assertion counts malformed values across ALL YEARS.",
        "-- It originally counted only publication_year >= 1989, which was right",
        "-- when the patch was 1989+ only; once the 770 pre-1989 OAI rows came into",
        "-- scope the assertion saw 2,629 of 3,399 repairs and aborted a dry run.",
        "-- The scope of the check has to track the scope of the patch.",
        ...
        r" where doi is not null and doi !~ '^10\.[0-9]{4,9}/';",
        "",
    ]
    for r in sorted(emit, key=lambda r: (r["tier"], r["id"])):
        lines.append(
